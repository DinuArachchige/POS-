...
export class PlaceOrderComponent implements OnInit {
  ...
  constructor ( private pOS: PlaceOrderServiceService, private cS: CustomerService , private iSer: ItemService, private elem: ElementRef) {
  }
  ...
  searchItems(event: any): void {
    this.iSer.searchItem(event.target.value).subscribe(
      (result) => {
  ...
  getItemCode() {
    this.iSer.getAllItems().subscribe(
      (result) => {
        this.items = result;
      }
    );
  ...
  searchCustomer(event: any): void {
    this.cS.searchCustomer(event.target.value).subscribe(
      (result) => {
  ...
  addOrder(): void {
    ...
    this.pOS.placeOrder(this.placeOrder).subscribe(
      (result) => {
        if (result) {
